 /*
    ...
    EX)
    - 회원탈퇴시 기존의 회원테이블에 데이터 DELETE 후 곧바로 탈퇴된 회원들만 따로 보관하는 
    테이블에 자동으로 INSERT 처리
    - 신고횟수가 일정 수를 넘었을 때 해당 회원을 자동으로 블랙리스트로 처리
    - 입출고에 대한 데이터가 기록(INSERT)될 때마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야 될때
 ...
 */
 -- 1. 문장 트리거
 -- EMPLOYEE 테이블에 새로운 행이 INSERT 될때
 -- '신입 사원이 입사했습니다.' 메시지를 자동으로 출력하는 트리거 생성
