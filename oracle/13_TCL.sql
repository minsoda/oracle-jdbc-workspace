 /*
    TCL(TRANSACTION CNTROL LANGUAGE)
    - 트랜잭션을 제어하는 언어
    ...
    -EX) ATM기에서 현금 출력
      ...
      5. 현금 인출
      6. 완료
    - 각각의 업무들을 묶어서 하나의 작업 단위로 만드는 것을 트랜잭션이라고 함.
    ...
    SAVEPOINT
    - 저장점을 지정하고 ROLLBACK 진행 시 전체 작업을 ROLLBACK 하는 것이 아닌 SAVEPOINT까지의 일부만 롤백한다.
 ...
 */
 ...
 ROLLBACK TO SP1;
 SELECT * FROM EMP_TEST; -- EMP_ID = 200 살아남
 ...
 -- DDL 구문을 실행하는 순간 기존 메모리 버퍼에 임시 저장된 변경사항들이 무조건 DB에 반영됨.
 -- 강제 COMMIT 됨
 CREATE TABLE TEST(
 TID NUMBER
 );
 ...
 ROLLBACK;
 SELECT * FROM EMP_TEST; -- EMP_ID = 202 삭제되어있음
 ...
 INSERT INTO BANK VALUES('김민소', '신한은행', 500000);
  ...
  COMMIT; -- 오라클이라서 COMMIT을 해줘야 이클립스에서 보여짐
