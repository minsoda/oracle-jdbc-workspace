...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_CODE = D.DEPT_ID(+); -- 기준으로 삼고자하는 테이블의 반대편, 테이블의 컬럼 뒤에(+) 붙이기
...
ANSI 구문으로는 JOIN ON으로만 사용가능! (USING은 사용 불가)
*/
-- 사원명, 급여, 급여레벨
--> ANSI 구문
...
*/
-- 사원사번, 사원명, 사원부서 코드, 사수사번, 사수명, 사수부서코드 조회
-->> 오라클 구문
SELECT E.EMP_ID 사원사번 , E.EMP_NAME 사원명 , E.DEPT_CODE 사원부서코드, 
       M.EMP_ID 사수사번 , M.MANAGER_ID 사수명, M.DEPT_CODE 사수부서코드
...
-- 2. 70년대생 이면서 여자이고, 성이 전씨인 직원들의 직원명, 주민번호, 부서명, 직급명을 조회
...
SELECT EMP_NAME, BONUS, TO_CHAR(SALARY*12, 'FM99,999,999'), DEPT_TITLE, LOCAL_NAME
...
-- 5. 각 부서별 평균 급여를 조회하여 부서명, 평균 급여(정수 처리) 조회
SELECT NVN(DEPT_TITLE, '부서없음'), FLOOR(AVG(NVL(SALARY, 0))
FROM EMPLOYYE, DEPARTMENT
GROUP BY DEPT_TITLE
GROUP BY DEPT_TITLE NULLS FIRST;

-- 6. 각 부서별 총 급여의 합이 1000만원 이상인 부서명, 급여의 합을 조회
SELECT DEPT_TITLE, TO_CHAR(SUM(SALARY), FM99,999,999)
FROM EMPLOYEE , DEPARTMENT
GROUP BY DEPT_TITLE
HAVING SUM(SALARY) >= 1000000;

-- 7. 사번, 직원명, 급여 등급, 구분을 조회
 SELECT EMP_ID 사번, EMP_NAME 직원명, JOB_NAME 직급명, SAL_LEVEL "급여 등급",
 CASE 
 WHEN SAL_LEVER = 'S1' OR 'S2' THEN '초급'
 WHEN SAL_LEVER = 'S3' OR 'S4' THEN '고급'
 ELSE '중급'
 END "구분"
 FROM EMPLOYEE E
 JOIN JOB J ON  (E.JOB_CODE = J.JOB_CODE)
...
SELECT EMP_NAME 직원명, JOB_NAME 직급명, DEPT_COD 부서코드, DEPT_TITLE 부서명
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
WHERE DEPT_TITLE = '해외영업팀';
