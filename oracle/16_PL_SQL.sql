...
DECLARE
    EID NUMBER; 
    ENAME VARCHAR2(20);
    PI CONSTANT NUMBER := 3.14; -- 상수값, 초기화
BEGIN
    EID := &번호;
    ENAME := '&이름'; -- SCANNER 같은 아이
...
BEGIN
    ...
    -- 문제로 풀어보기!
    -- 레퍼런스 타입의 변수로 EID, ENAME, JCODE, DTITLE, SAL를 선언하고 각 변수의 
    -- 자료형은 EMPLOYEE 테이블의 EMP_ID, EMP_NAME, JOB_CODE, SALARY 컬럼과 
    -- DEPARTMENT 테이블의 DEPT_TITLE 컬럼의 자료형을 참조
    -- 사용자가 입력한 사번과 일치하는 사원을 조회하는 후 조회 결과를 출력
    ...
    DECLARE
    EMP EMPLOYEE%ROWTYPE; -- 컬럼의 타입들을 전부 들고옴
    ...
    */
    ...
    -- 단, 보너스를 받지 않는 사원은 보너스 출력 전에 '보너스를 지급받지 않는 사원입니다' 라는 문구를 출력
     ...
     SET SERVEROUTPUT ON; -- 화면에 보여주는 구문
    ...
    IF (BONUS = 0) THEN
    DBMS_OUTPUT.PUT_LINE('보너스를 지급하지 않는 사원입니다.' );
    END IF;
    ...
    */
    ...
    -- 일반 타입변수 TEAM을 데이터 타입 VARCHAR2(10)으로 선언하고
    -- NCODE 값이 KO일 경우 -> TEAM '국내팀' 대입
    --             그게 아닐경우 - > TEAM  '해외팀' 대입
    -- 사번(EID), 이름(ENAME), 부서(DTITLE), 소속(TEAM)출력
 ...
 ELSE 
    ...
    /*
        1-3) IF~ELSE IF~ELSE 구문
        [표현식]
        IF 조건식 1 THEN 실행내용1
        ELSIF 조건식 2 THEN 실행내용2
        ...
        [ELSE 실행내용 N]
        END IF;
    */
    -- 사용자에게 점수를 입력받아 SCORE 변수(데이터타입 : NUMBER)에 저장한 후 학점은 입력된 점수에 따라 
    -- GRADE 변수(데이터 타입 : CHAR(1)에 저장
    -- 90점 이상은 'A', 80점 이상은 'B', 70점 이상은 'C',  60점 이상은 'D', 60점 미만은 'F'
    -- 출력은 '당신의 점수는 95점이고, 학점은 A 학점입니다.' 출력
    ...
    DBMS_OUTPUT.PUT_LINE('당신의 점수는 : ' || SCORE || '학점은 ' || GRADE || '입니다.');
    END;
    /
     ...
     */
     ...
     -- DEPT_CODE에 따라서 알맞는 부서를 출력
     -- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, 
     -- D4인 경우 국내 영업부, D5인 경우 해외영업1부, D6인 경우 해외영업2부, 
     -- D7인 경우 해외영업3부, D8인 경우 기술지원부, D9인 경우 총무부, 나머지는 부서없음.
     -- 출력은 사번, 이름, 부서 코드, 부서가 출력
...
--    2)EXIT WHEN 조건식;
   ...
   */
    ...
    */
    ...
    /*
        ...
        * 오라클에서 미리 정의되어 있는 시스템 예외
        ...
        - TOO_MANY_ROWS : SELECT한 결과가 한 행이 리턴되어야 하는데 여러행일 경우
    ...
    */
    ...
    -- TOO_MANY_ROWS : SELECT한 결과가 여러 행인 경우 (한 행이 리턴 되어야하는 경우)
    DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ...
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('에러가 발생했습니다. 관리자에게 문의해주세요.');
    END;
    /
