...
-- 사원의 수, 보너스를 받는 사원의 수 , 총 급여, 평균 급여, 최저 급여 조회
-- (부서별 오름차순 정렬, 부서없음이 맨 위에)
...
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE NULLS FIRST; -- 1일때는 적용이 되지않아 DEPT_CODE로 작성해줘야함
...
/*
GROUPING
- ROLLUP 이나 CUBE에 의해 산출된 값이 해당 컬럼의 집합의 산출물이면 0을 반환, 아니면 1을 반환하는 함수
*/
...
-- 1. UNION
-- 부서 코드가 D5인 사원 또는 급여가 300만원 초과인 사원들의 사번, 사원명, 부서코드, 급여를 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION
